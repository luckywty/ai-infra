...
def get_token_throughput_latencies(
    ...
    Args:
        model: 要查询的模型的名称。
        mean_input_tokens: 请求提示中要发送的 token 的平均数量。
        stddev_input_tokens: 请求提示中要发送的 token 数量的标准差。
        mean_output_tokens: 每个请求要生成的 token 的平均数量。
        stddev_output_tokens: 每个请求要生成的 token 数量的标准差。
        additional_sampling_params: 随请求一起发送的额外采样参数。
            更多信息请参阅 LLM API 关于补全（completions）的文档。
        num_concurrent_requests: 要发起的并发请求数量。增大此值会增加负载，反之亦然。
        test_timeout_s: 在报告结果之前运行测试的时间长度。
        llm_api: 要使用的 LLM API 的名称。可以是 "openai" 或 "litellm"。
    ...
    for i in range(max_num_completed_requests):
        ...
        num_output_tokens_list.append(num_output_tokens)# 随机输出token数

        prompts.append(randomly_sample_sonnet_lines_prompt( #随机提示词 
    ...
    def launch_request(thread_index):
        ...
        print("request_metrics NUM_OUTPUT_TOKENS before overwrite:", request_metrics[common_metrics.NUM_OUTPUT_TOKENS])
    threads = []
...
def run_token_benchmark(
    ...
    Args:
        ...
        model: 要查询的模型的名称。
        max_num_completed_requests: 在结束测试之前要完成的请求数量。
        test_timeout_s: 在报告结果之前运行测试的时间长度。
        num_concurrent_requests: 要发起的并发请求数量。增大此值会增加负载，反之亦然。
        mean_input_tokens: 请求提示中要发送的 token 的平均数量。
        stddev_input_tokens: 请求提示中要发送的 token 数量的标准差。
